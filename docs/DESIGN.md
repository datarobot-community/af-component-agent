...
| `use_agent_memory` | Agent memory provider choice: `none`, `mem0`, or `datarobot_memory_service`. Only prompted when `ENABLE_AGENTIC_MEMORY_API` is enabled on the connected DataRobot tenant. |
