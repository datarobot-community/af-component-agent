...
This is required for the DataRobot Memory Service provider. Mem0 gets the same runtime parameter (it's provisioned whenever `use_agent_memory` is `mem0` or `datarobot_memory_service`), but doesn't need it&mdash;Mem0 talks to Mem0's own SaaS API (or `AGENT_MEMORY_LLM_BASE_URL`-style host overrides) and never resolves a DataRobot API endpoint for its backend, so the parameter is simply unused there.

The memory client (`datarobot-genai` >= 0.29.12) reaches the DataRobot Memory Service at `{endpoint}/memory/{agent_memory_space_id}/`. It prefers `DATAROBOT_PUBLIC_API_ENDPOINT` over `DATAROBOT_ENDPOINT` when both are set, because on-prem deployments often point `DATAROBOT_ENDPOINT` at an internal, cluster-only address while `DATAROBOT_PUBLIC_API_ENDPOINT` holds the externally reachable one that the memory service actually requires.

To make sure memory always has an endpoint it can use, `infra/infra/<agent_app_name>_infra/base.py` forwards `DATAROBOT_PUBLIC_API_ENDPOINT` as a runtime parameter whenever `use_agent_memory` is `mem0` or `datarobot_memory_service`. It is set from, in order:
...
2. `DATAROBOT_PUBLIC_API_ENDPOINT`&mdash;added for either provider (string type) when either `DATAROBOT_PUBLIC_API_ENDPOINT` or `DATAROBOT_ENDPOINT` is set in the Pulumi deploy environment; required by DataRobot Memory Service, unused by Mem0 (see [DataRobot API endpoint for memory](#datarobot-api-endpoint-for-memory-datarobot_public_api_endpoint) above).
