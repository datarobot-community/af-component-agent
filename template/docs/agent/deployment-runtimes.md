...
| Workload logs show `could not create 'agent.egg-info': Permission denied` | The agent is installed at container start, where the application root is read-only. `workload/build.cfg` redirects setuptools' build output to a writable directory; check that it is still present and uploaded (it must not be excluded by `.wapiignore`), and that `workload/run_server.sh` still points `DIST_EXTRA_CONFIG` at it. |
