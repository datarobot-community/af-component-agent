...
On the **Workload API** runtime (`ENABLE_AGENT_ON_WORKLOAD_API=true`), setting this flag also adds an optional-auth route for `/a2a/.well-known/agent-card.json` to the workload artifact specification during deploy.
