...
| `DATAROBOT_PUBLIC_API_ENDPOINT` | string | `use_agent_memory: datarobot_memory_service`, and either `DATAROBOT_PUBLIC_API_ENDPOINT` or `DATAROBOT_ENDPOINT` is set in the Pulumi environment | [Agent memory: Configuration and runtime parameters](./agent-memory.md#configuration-and-runtime-parameters) |
