...
| `MCP_DEPLOYMENT_ID`, `EXTERNAL_MCP_URL`, `EXTERNAL_MCP_HEADERS`, `EXTERNAL_MCP_TRANSPORT` | string | `get_mcp_runtime_parameters_from_env()` in `infra/infra/<agent_app_name>.py` | MCP server connection details, when an MCP deployment or external MCP URL is configured. See [MCP server](../mcp-server.md). |
